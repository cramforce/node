...
function WorkerChild (eventDest, filename) {
  ...
  this.child.addListener("output", function (data) {
    sys.error("From worker " + data);
    self.handleData(data);
  });
  
  this.child.addListener("error", function (data) {
    if(data !== null) {
      sys.error(self.child.pid + ": "+ data);
    }
  });
  
  this.child.addListener("exit", function (code) {
    sys.error(self.child.pid + ": exit "+code);
  });
...
WorkerChild.prototype = {
  ...
  postMessage: function (message) {
    var self = this;
    sys.error("Sending data "+message);
    self.child.write(message+MESSAGE_SPLITTER, "utf8");
    setTimeout(function () {
      self.child.write(""+MESSAGE_SPLITTER)
    }, 100)
  },
...
function WorkerProcess(eventDest) {
  ...
  process.stdio.addListener("data", function (data) {
    sys.error("Process receiving data "+data);
    self.handleData(data);
  });
  this.buffer = "";
}
...
WorkerProcess.prototype = {
  postMessage: function (message) {
    sys.error("Process posting message "+message);
    sys.print(message+MESSAGE_SPLITTER);
    setTimeout(function () {
      sys.print(""+MESSAGE_SPLITTER);
    }, 100)
  },
